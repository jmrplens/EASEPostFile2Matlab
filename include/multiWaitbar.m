function cancel = multiWaitbar( handles,label, varargin )
...
% Modificaciones para CATT2Matlab
global titleprogress
global timedays
global timehours
global timemins
global timesecs
global timesec 
titleprogress = handles.LPROGRESSTITLE;
timedays = handles.LTIMEDAYS;
timehours = handles.LTIMEHOURS;
timemins = handles.LTIMEMINS;
timesecs = handles.LTIMESECS;
timesec = handles.LTIMESEC;
...
% Check basic inputs
narginchk( 1, inf); % - kept for backwards compatibility
if ~ischar( label )
...
clearvars -global titleprogress timedays timehours timemins timesecs timesec
end % multiWaitbar
...
if remainingtime > 172800 % 2 days
    eta = [sprintf( '%d ', round(remainingtime/86400) ),timedays];
else
    if remainingtime > 7200 % 2 hours
        eta = [sprintf( '%d ', round(remainingtime/3600) ),timehours];
    else
        if remainingtime > 120 % 2 mins
            eta = [sprintf( '%d ', round(remainingtime/60) ),timemins];
        else
            % Seconds
            remainingtime = round( remainingtime );
            if remainingtime > 1
                eta = [sprintf( '%d ', remainingtime ),timesecs];
