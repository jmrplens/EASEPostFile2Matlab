function representarCt(hObject,handles)
% Reprsentacion del parametro claridad
...
% Claridad EASE
% Muestra los puntos
plot(Dist,Urec-Prec,'^b','MarkerFaceColor','b'),hold on
% Muestra la curva
Curv(1)=plot(Distplot,Uplot-Pplot,'b','LineWidth',1.2);
...
% Leyenda
leyenda{1} = sprintf('C_{%s} - EASE',num2str(Rango));
...
if get(handles.teoriacorregida,'value')==1
    ...
    Curv(2) = plot(Distplot,Pdeplot - Plateplot,'-.r','LineWidth',1);

    % Leyendas
    leyenda{2} = sprintf('C_{%s} - Teórico',num2str(Rango));
...
ylabel(sprintf('C_{%s} (dB)',num2str(Rango)))
xlabel('Distancia (m)')
title({sprintf('C_{%s}',num2str(Rango));...
